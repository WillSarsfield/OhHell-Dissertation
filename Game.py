    ...
    def round(self, trump, first, handSize):
        ...
        for i in range(first , first + len(self.playerList)):
            ...
            if i != first + len(self.playerList) - 1:
                if i == first:
                    self.playerList[i % len(self.playerList)].playBid(handSize + 1, handSize, trump, True, len(self.playerList), bids, i % len(self.playerList)) #can make bid, argument passed represents a bid that is banned (14 passed as it is an unbiddable number)
                else:
                    self.playerList[i % len(self.playerList)].playBid(handSize + 1, handSize, trump, False, len(self.playerList), bids, i % len(self.playerList))
                ...
                if bidTotal < handSize + 1: #calculates the bid that is banned for the final player
                    self.playerList[i % len(self.playerList)].playBid(handSize - bidTotal, handSize, trump, False, len(self.playerList), bids, i % len(self.playerList))
                else:
                    self.playerList[i % len(self.playerList)].playBid(handSize + 1, handSize, trump, False, len(self.playerList), bids, i % len(self.playerList))
                print("player " + str((i % len(self.playerList))+1) + " bid: " + str(self.playerList[i % len(self.playerList)].getBid()))
                bids.append(self.playerList[i % len(self.playerList)].getBid())
        ...
        for i in range(0, handSize):
            ...
            leadCard = self.playerList[first].playOption(options, cardList, trump, len(self.playerList), bids, scores, first) #lead player chooses a play from its options
            print(leadCard)
            ...
            for i in range(first + 1 , first + len(self.playerList)): #iterate over remaining players choices in order ascending from first player
                ...
                card = self.playerList[i % len(self.playerList)].playOption(options, cardList, trump, len(self.playerList), bids, scores, i % len(self.playerList)) #player chooses a play from its options
                print(card)
